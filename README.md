...
**本地还有别的中转/网关工具（如 cc-bridge、CC Switch 等），代理连不上怎么办？**

SubFuse 启动后只占用 **127.0.0.1:7890** 一个代理端口。任何依赖本地代理的
配套工具，其 `proxy` / 代理端口配置都必须是 `http://127.0.0.1:7890`。
例如 cc-bridge（Claude 本地网关，默认 127.0.0.1:8567）如果还指向旧的
7897 端口，会报 `Connection refused`，导致 Claude 等应用无法出网。

**为什么网页版 AI 偶尔被提示“异常流量”？**

SubFuse 已默认关闭 IPv6（`ipv6: false`），保证 IPv4/IPv6 出口一致，
避免 Google 等站点因看到两个不同地区的出口 IP 而判定异常流量。若仍遇到，
请确认没有其他代理客户端在同时运行，并把节点换到干净出口。

**国内 App（酷狗、微信等）卡顿？**

内置国内常用应用域名直连清单，并支持实时自动学习：检测到国内 IP 误走代理时
会自动加入直连并热更新。若个别 App 仍异常，把它的域名反馈给项目维护即可
加入清单。
